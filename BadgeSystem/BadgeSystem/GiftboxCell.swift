...
class GiftboxCell: UITableViewCell {
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String!) {
        ...
        catImg = UIImageView()
        catImg?.frame = CGRect(x:10, y:10, width:40, height:40)
        catImg!.backgroundColor = UIColor.clear
        catImg!.image = UIImage(named: "academy")
        trans!.addSubview(catImg!)
        ...
        oppLabel = UILabel()
        oppLabel = UILabel(frame: CGRect(x:60, y:0, width:trans!.frame.width-60, height:trans!.frame.height))
    ...
    func fillWith(category:String, name:String, from: String, time: Int) {
        catImg?.image = UIImage(named:category)
        oppLabel!.text = name
